...
app = (
    lambda pid: subprocess.check_output(["ps", "-p", pid, "-o", "comm="])
    .decode("utf-8")
    .strip()
)
...
def read_windows():
    ...
    relevant = [
        [w[2], [int(n) for n in w[3:7]]] for w in w_list if check_window(w[0]) == True
    ]
...
def open_appwindow(app, x, y, w, h):
    ...
    while t < 30:
        ...
        procs = [
            [(p, w[0]) for p in get("ps -e ww").splitlines() if app in p and w[2] in p]
            for w in ws2
        ]
        if len(procs) > 0:
            time.sleep(0.5)
...
def run_remembered():
    ...
    try:
        ...
        for l in lines:
            l[1] = str(int(l[1]) - res[0])
            l[2] = str(int(l[2]) - res[1] - 24)
